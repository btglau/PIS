...
if k >= 4
    for iRoot = 4:k
        ...
        ind = ((iRoot-1)*nOrdersPerRoot+1:iRoot*nOrdersPerRoot)';
        x(ind) = 2*x(((iRoot-2)*nOrdersPerRoot+1:(iRoot-1)*nOrdersPerRoot)')- x(((iRoot-3)*nOrdersPerRoot+1:(iRoot-2)*nOrdersPerRoot)');
        % find the remaining zeros
        x(ind) = arrayfun(@(n, x0) findzero(n, k, x0, kind), n(:), x(ind));
    end
end
